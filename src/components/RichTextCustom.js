  ...
  // Generate links to Prismic Documents as <Link> components
    if (type === 'hyperlink') {
      ...
      if (element.data.link_type === 'Document') {
          ...
          const target = element.data.target ? { 'target': element.data.target, 'rel':'noopener noreferrer' } : {}
    ...
    if (type === 'image') {
      ...
      if (element.linkTo.link_type === 'Document') {
          ...
          const target = element.linkTo.target ? { 'target':element.linkTo.target, 'rel':'noopener noreferrer' } : {}
